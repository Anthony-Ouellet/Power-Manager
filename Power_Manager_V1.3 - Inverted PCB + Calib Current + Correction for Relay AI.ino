...
float PowerManagerScale = 0.01215;
float PowerManagerOffset = -6.2031;
float TurbineScale = 0.08697;
float TurbineOffset = -44.53954;
float WinchScale = 0.07301;
float WinchOffset = -37.49683;
int navg = 15;
...
void collectPowerData() {
      ...
      if ((PowerResult[i] == 0x3E || PowerResult[i] == 0x3F || PowerResult[i] == 0x40 || PowerResult[i] == 0x41 || PowerResult[i] == 0x42)  && PowerResult[i-9] == 0x0D && PowerResult[i-11] == 0X01){
        ...
        if (ASolar < 1){
          ...
          for(int t = 0; t<10; t++){
              ...
              ASolar = ASolar + analogRead(A4);
              //Serial1.println((String)"ASolar from ACS712 = "+ ASolar);
              //Serial1.flush();
            }
          ASolar = ASolar / 10;
          ASolar = (ASolar * 0.0853) - 43.569;
          newASolar = 1;
        }
      }
      i++;
  ...
  for (int t=0; t<1; t++){
    ...
    for (int j = 0; j < navg; j++){
      ...
      if (AWinch > 511 && AWinch < 515){ //Replace 0 values with noise around 513 (0A)
        k++;
        }
  ...
  for (int t = 0; t<1; t++){
    ...
    //Serial1.println(sum/navg);
    if (k < (navg/3)){APowerManager = (PowerManagerScale * (sum / navg) + PowerManagerOffset) - 0.08;} //0.08 is the Power Manager draws when collecting information
